...
class SpaceBehaviour extends Behaviour {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    for (final Star star in stars!) {
      ...
      final double x = star.targetPosition.dx / star.distance * 1.02;
      final double y = star.targetPosition.dy / star.distance * 1.02;
      final double z = 1.0 / star.distance * 6.0 + 1.0;
      paint.strokeWidth = z;
  ...
  Widget builder(
    ...
    return GestureDetector(
      ...
      onPanUpdate: (DragUpdateDetails details) => _updateCenter(
        context,
        details.globalPosition,
      ),
      onTapDown: (TapDownDetails details) => _updateCenter(
        context,
        details.globalPosition,
      ),
...
class ChildFlySpaceBehaviour extends SpaceBehaviour {
  ...
  bool tick(double delta, Duration elapsed) {
    if (_flying) {
      _childZ = math.max(0, _childZ - 50 * delta);
