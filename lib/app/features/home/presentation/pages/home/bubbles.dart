...
class BubbleOptions {
  ...
  const BubbleOptions({
    ...
    this.popRate = 150.0,
  })  : assert(bubbleCount >= 0, 'bubbleCount >= 0'),
        assert(minTargetRadius > 0, 'minTargetRadius > 0'),
        assert(maxTargetRadius > 0, 'maxTargetRadius > 0'),
        assert(growthRate > 0, 'growthRate > 0'),
        assert(popRate > 0, 'popRate > 0');
...
class BubblesBehaviour extends Behaviour {
  ...
  set options(BubbleOptions value) {
    ...
    final BubbleOptions? oldOptions = _options;
    _options = value;
  ...
  @override
  void init() => bubbles = generateBubbles(options.bubbleCount);
  ...
  /// the options change
  @protected
  List<Bubble> generateBubbles(int num) => List<Bubble>.generate(num, (_) {
  ...
  void _initBubble(Bubble bubble) {
    ...
    final double deltaTargetRadius =
        options.maxTargetRadius - options.minTargetRadius;
    bubble.targetRadius =
  ...
  void _popBubble(Bubble bubble, bool wasTap) {
    bubble
      ..popping = true
      ..radius = 0.2 * bubble.targetRadius
      ..targetRadius *= 0.5;
    if (onPop != null) {
      onPop!(wasTap);
    }
  ...
  void onOptionsUpdate(BubbleOptions? oldOptions) {
    ...
    if (bubbles!.length > options.bubbleCount) {
      ...
      final List<Bubble> newBubbles = generateBubbles(numToSpawn);
    ...
    for (final Bubble bubble in bubbles!) {
      ...
      if (!bubble.popping) {
        ...
        canvas
          ...
          ..drawLine(
            bubble.position + Offset(0, bubble.radius!),
            bubble.position + Offset(0, bubble.targetRadius),
            paint,
          )
          ..drawLine(
            bubble.position + Offset(0, -bubble.radius!),
          ...
          ..drawLine(
            bubble.position + Offset(bubble.radius!, 0),
            bubble.position + Offset(bubble.targetRadius, 0),
            paint,
          )
          ..drawLine(
            bubble.position + Offset(-bubble.radius!, 0),
